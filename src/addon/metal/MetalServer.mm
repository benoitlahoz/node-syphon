...
void MetalServerWrapper::PublishSurfaceHandle(const Napi::CallbackInfo& info)
{
    ...
    if (!m_first_check_passed)
    {
        ...
        if (!IS_SIZE(info[3]))
            Napi::TypeError::New(env,
              "4th parameter (textureDimensions) must be a size").ThrowAsJavaScriptException();

        if (!info[4].IsBoolean())
            Napi::TypeError::New(env,
              "5th parameter (flipped) must be a boolean").ThrowAsJavaScriptException();
    ...
    Napi::Object regionObj = info[2].As<Napi::Object>();
    ...
    Napi::Object sizeObj = info[3].As<Napi::Object>();
