...
/* -------------------------------------------------------------------------- */
/*  MetalServerWrapper::publishSubSurfaceHandle — publish only a sub-region   */
/*  of an IOSurface-backed texture.                                           */
/*                                                                            */
/*  JS signature                                                               */
/*    publishSubSurfaceHandle(                                                */
/*        handleBuf : Buffer,            // IOSurfaceRef as Node Buffer       */
/*        imageRegion : {x,y,width,height},                                   */
/*        textureDims : {width,height},  // original surface size (optional)  */
/*        flipped : boolean              // Syphon’s usual “flipped” flag     */
/*    )                                                                        */
/* -------------------------------------------------------------------------- */
void MetalServerWrapper::PublishSubSurfaceHandle(const Napi::CallbackInfo &info)
{
    Napi::Env env{info.Env()};
    Napi::HandleScope scope{env};
    ...
    if (!m_first_check_passed)
    {
        if (info.Length() != 4)
            Napi::TypeError::New(env,
               "publishSubSurfaceHandle expects (Buffer, rect, size, bool)")
               .ThrowAsJavaScriptException();
        ...
        if (!IS_SIZE(info[2]))
            Napi::TypeError::New(env,
               "3rd parameter (textureDims) must be a size").ThrowAsJavaScriptException();
    ...
    const size_t fullW = IOSurfaceGetWidth (ioSurface);
    const size_t fullH = IOSurfaceGetHeight(ioSurface);

    MTLTextureDescriptor *srcDesc =
        [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatBGRA8Unorm
                                                           width:fullW
                                                          height:fullH
                                                       mipmapped:NO];
    srcDesc.usage           = MTLTextureUsageShaderRead | MTLTextureUsageShaderWrite;
    srcDesc.storageMode     = MTLStorageModeShared;
    srcDesc.resourceOptions = MTLResourceCPUCacheModeDefaultCache | MTLResourceStorageModeShared;

    id<MTLTexture> srcTexture =
        [m_device newTextureWithDescriptor:srcDesc iosurface:ioSurface plane:0];

    if (!srcTexture)
    {
        Napi::Error::New(env, "Failed to wrap IOSurface in MTLTexture")
            .ThrowAsJavaScriptException();
        return;
    }

    /* ───────────── Make a crop texture sized exactly to imageRegion ───────────── */
    const NSUInteger cropW = (NSUInteger)imageRegion.size.width;
    const NSUInteger cropH = (NSUInteger)imageRegion.size.height;

    MTLTextureDescriptor *cropDesc =
        [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatBGRA8Unorm
                                                           width:cropW
                                                          height:cropH
                                                       mipmapped:NO];
    cropDesc.usage       = MTLTextureUsageShaderRead |
                           MTLTextureUsageRenderTarget;
    cropDesc.storageMode = MTLStorageModePrivate;     // GPU-only is fine

    id<MTLTexture> cropTexture = [m_device newTextureWithDescriptor:cropDesc];
    if (!cropTexture)
    {
        [srcTexture release];
        Napi::Error::New(env, "Failed to allocate crop texture")
            .ThrowAsJavaScriptException();
        return;
    }

    /* ───────────── Copy sub-region from IOSurface texture into cropTex ───────── */
    id<MTLCommandBuffer> cmd  = [m_queue commandBuffer];
    id<MTLBlitCommandEncoder> blit = [cmd blitCommandEncoder];

    MTLOrigin srcOrigin = { (NSUInteger)imageRegion.origin.x,
                            (NSUInteger)imageRegion.origin.y, 0 };
    MTLSize   srcSize   = { cropW, cropH, 1 };
    ...
    [blit copyFromTexture:srcTexture
             ...
             sourceOrigin:srcOrigin
               sourceSize:srcSize
               toTexture:cropTexture
        destinationSlice:0
        destinationLevel:0
       destinationOrigin:(MTLOrigin){0,0,0}];
    [blit endEncoding];

    /* ───────────── Publish the cropped texture (region is now full-frame) ─────── */
    [m_server publishFrameTexture:cropTexture
                 onCommandBuffer:cmd
                     imageRegion:NSMakeRect(0, 0, cropW, cropH)
                         flipped:flipped];
...
Napi::Object MetalServerWrapper::Init(Napi::Env env, Napi::Object exports)
{
  ...
  Napi::Function func = DefineClass(env, "MetalServer", {
    ...
    InstanceMethod("publishSubSurfaceHandle", &MetalServerWrapper::PublishSubSurfaceHandle),
